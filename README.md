...
`kry build --format=elf --target=linux-x64` and `kry build --format=exe --target=windows-x64` produce real, runnable executables through a C-based ahead-of-time backend. The checked program is lowered to C and compiled with the host toolchain (`cc`/`gcc`/`clang` for Linux, `x86_64-w64-mingw32-gcc` for Windows), so the output runs the full checked language with byte-for-byte identical output to the interpreter. This includes functions, recursion, control flow, `match`, structs, enums, collections, `Option`/`Result`, `defer`, JSON, crypto, the filesystem surface, `Shared[T]` cells, `Actor[T]` mailboxes, `TaskGroup`, threads, `await`, and runtime polymorphic dispatch. `--format=c` emits the generated C source. Constructs that are genuinely unsupported (HTTP, WebSockets, Windows APIs) are rejected during the build instead of being silently degraded.
