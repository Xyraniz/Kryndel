...
`kry build --format=elf --target=linux-x64` and `kry build --format=exe --target=windows-x64` produce real, runnable executables through a C-based ahead-of-time backend. The checked program is lowered to C and compiled with the host toolchain (`cc`/`gcc`/`clang` for Linux, `gcc`/MinGW for Windows); on Windows, a Linux target can use the configured WSL Ubuntu cross-compiler. The output runs the checked core language with byte-for-byte parity for the supported native surface. `--format=c` emits the generated C source. Host integrations that require Go runtime libraries or platform frameworks (HTTP/TLS, WebSockets, SQLite, FFI, regex handles, camera/display APIs, raw sockets, geolocation, process enumeration, and Windows APIs) are rejected by the AOT backend with an explicit builtin diagnostic instead of being silently degraded; the interpreter remains the complete host-integration backend.
