...
Adds ten JSON REST methods adapted from discord.py-self: invite and widget lookup, sticker and sticker-pack lookup, guild member-verification, join-request lookup and guild listing, account settings and tracking consent, and friend suggestions. Snowflake and invite-code path/query values are validated before requests.
