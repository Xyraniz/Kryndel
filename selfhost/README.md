...
The dynamic KIR stage additionally accepts top-level and nested `let`/`const`, mutable Int/Bool/UInt slots, assignments, checked signed arithmetic, wrapping fixed-width unsigned arithmetic, comparisons, bitwise operations, `if`/`else`, `while`, `break`, `continue`, scalar functions with up to six SysV AMD64 register parameters, scalar `Int`/`Bool`/`UInt` values, static `String` objects passed and returned by pointer, scalar function-local declarations and assignments, static display values, and dynamic `Int`/`UInt`/`String` values inside `print`/`println`. It emits stack loads/stores, decimal integer conversion, relative branches, calls/returns, RIP-relative references to immutable string objects, overflow traps, a Linux `mmap`-backed String allocation/copy runtime, and Linux syscalls directly from Kryndel. Other dynamic String operations, collection/resource parameters and returns, non-scalar function locals, `for`, `match`, heap values beyond concatenation, and non-Linux targets remain explicit rejection points.

Unsupported arbitrary calls, dynamic String operations other than `+`, non-scalar function parameters/return values/locals, heap values beyond the String-concatenation allocation path, and non-Linux targets are rejected with explicit errors. This restriction is intentional while the lowering is being expanded.
