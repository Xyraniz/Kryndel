...
Stage 15 adds the first direct-ELF host-runtime boundary needed by a bootstrap executable: `process_args`, Linux syscall-backed `fs_read_text`, `bytes(Array[Int])`, and syscall-backed `fs_write_bytes`. The regression exercises both a successful write and a rejected path, and verifies the resulting bytes. These builtins are native direct-ELF support; the self-hosted KIR emitter still needs a matching lowering before the compiler itself can be emitted without the Go interpreter.
