...
| --- | --- | --- | --- | --- |
| 0 | Go reference compiler built from `cmd/kry` and `internal/kry`. | Linux amd64 host `kry` executable. | This is the trusted reference and may use Go. Record its exact build command, executable hash, and Go version. | Pinned in schema 2 of `selfhost/bootstrap.lock.json`: Go 1.26.0, `CGO_ENABLED=0 go build -trimpath -ldflags='-s -w' -o <tmp>/kry ./cmd/kry`, and the executable SHA-256. Stage 36 builds that executable, verifies the hash, then uses its CLI to emit KIR and run `kir_backend.kry`. The source revision identifies the commit whose normalized compiler/backend/fixture inputs are hash-checked; the Stage 0 executable hash independently detects changes to the Go host compiler. |
...
The test builds the locked Go 1.26.0 Stage 0 executable, verifies its hash, and
uses its CLI to emit KIR and run `kir_backend.kry` to produce Stage 1 in a
