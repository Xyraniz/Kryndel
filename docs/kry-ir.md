...
The repository's self-hosted slices are `selfhost/elf_backend.kry` and `selfhost/dynamic_backend.kry`. The latter reads KIR v1, lowers stack-backed Int/Bool state, assignments, comparisons, `if`/`else`, `while`, loop control, static output, relative x86-64 branches, RIP-relative data references, and ELF64 headers using only Kryndel arrays and `UInt16/32/64`. `array_set` applies checked immutable patches after emission. The Go direct backend is the byte-level oracle for both slices; regression tests require byte-identical output. Unsupported functions, heap values, dynamic formatting, and non-Linux targets are rejected explicitly.
