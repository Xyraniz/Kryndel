...
The repository's self-hosted slices are `selfhost/elf_backend.kry`, `selfhost/dynamic_backend.kry`, and `selfhost/source_kir_compiler.kry`. The latter serializes its checked source subset to KIR JSON and feeds the dynamic backend. The dynamic backend reads KIR v1, lowers stack-backed Int/Bool/UInt state, assignments, checked signed and wrapping unsigned arithmetic, comparisons, `if`/`else`, `while`, loop control, scalar function calls using up to six SysV AMD64 registers and scalar returns in `RAX`, static and dynamic integer output, relative x86-64 branches, RIP-relative data references, and ELF64 headers using only Kryndel arrays and `UInt16/32/64`. `array_set` applies checked immutable patches after emission. The Go direct backend is the byte-level oracle for all three slices; regression tests require byte-identical output. Non-scalar parameters/returns, function-local declarations, heap values, and non-Linux targets are rejected explicitly.
