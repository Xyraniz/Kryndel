...
The repository's first self-hosted slice is `selfhost/elf_backend.kry`. It reads KIR v1, lowers static `let`/`const` display expressions and `print`/`println` calls, assembles the x86-64 `write`/`exit` sequence, constructs ELF64 headers with `UInt16/32/64`, and writes the executable through `fs_write_bytes`. The Go direct backend is the byte-level oracle for this slice; a regression test requires byte-identical output. Unsupported dynamic statements are rejected explicitly.
