...
| `build source.kry --format=elf-direct --target=linux-x64` | Use the dependency-free direct machine backend for the documented static-output and Int/Bool assignment/`if`/`while` slice; no C compiler is invoked. | `0` |
