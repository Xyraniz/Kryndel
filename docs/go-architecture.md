...
Artifacts use `KRYNATIVE4`: a fixed header, language-version metadata, explicit root entry, canonical remaining entry order, exact byte lengths, SHA-256 hashes, compiler and target identities, and strict trailing-byte rejection. The reader accepts KRYNATIVE3 as language version 1.0.0. An artifact is a source container that is rechecked by the same frontend before execution. Atomic replacement uses `os.CreateTemp`, sync, close, and rename in the same directory.
