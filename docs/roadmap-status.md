...
| Direct backend seed seam | `stdlib/core/backend.kry`, `backend-v1.md`, `backend-seed-v2.json`, backend/CLI regressions | Kryndel source and `tools/kry-seed` emit x86_64 Linux exit-status seeds for an empty main with codes 0..255; complete native backend, object format, linking, and compiler/runtime integration remain pending |
