...
The source-level filesystem boundary is now implemented as a verified bootstrap API, a first strict manifest reader exists in Kryndel source over that boundary, `stdlib/core/data.kry` provides bounded readers, a balanced string builder, and nominal toolchain records, and the source manifest module now emits canonical lockfile JSON for validated entries. These are executable compatibility seams under the Python VM, not native implementations. The next technical gate is full byte-for-byte differential parity for manifest diagnostics and a native byte-oriented verifier or SHA-256 implementation before replacing the remaining package tooling.
