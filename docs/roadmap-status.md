...
| 5. Kryndel-native SHA-256 or verifier | Structural verifier, KEXE framing, SHA-256, and general JSON source seams; schema/module decoding pending | `stdlib/core/bytecode.kry` validates normalized v1 records, `stdlib/core/artifact.kry` validates KEXE framing and canonicalizes checksum text, `stdlib/core/sha256.kry` verifies the extracted payload, and `stdlib/core/json.kry` parses nominal JSON values; Python still decodes production KEXE modules and owns artifact/package integration |
...
| Source JSON parser seam | `stdlib/core/json.kry`, `json-source-v1.json`, nominal-value regression | Kryndel source parses the tested JSON value grammar under the Python VM; schema validation, ModuleRecord construction, file input, and production KEXE decoding remain Python |
