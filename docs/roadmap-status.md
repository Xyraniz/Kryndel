...
| 5. Kryndel-native SHA-256 or verifier | Structural verifier and KEXE framing reader source seams; checksum/JSON pending | `stdlib/core/bytecode.kry` validates normalized v1 records and `stdlib/core/artifact.kry` validates KEXE magic/length and extracts fields; Python still parses JSON/KEXE and computes SHA-256 |
...
| Source KEXE framing reader | `stdlib/core/artifact.kry`, `kexe-reader-v1.json`, framing regression test | Kryndel source executed by Python VM; framing is source-owned for the tested bytes, while SHA-256, JSON/module decode, and production artifact CLI remain Python |
