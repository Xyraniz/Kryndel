...
| 5. Kryndel-native SHA-256 or verifier | Structural verifier, KEXE framing, SHA-256, general JSON, and bounded bytecode-schema source seams; full schema/module parity pending | `stdlib/core/bytecode.kry` validates normalized v1 records, `stdlib/core/artifact.kry` validates KEXE framing, `stdlib/core/sha256.kry` verifies extracted payloads, and `stdlib/core/json.kry` builds a verifier-compatible subset; Python still decodes production KEXE modules and owns artifact/package integration |
...
| Source JSON parser seam | `stdlib/core/json.kry`, `json-source-v1.json`, `bytecode-schema-source-v1.json`, JSON/schema regressions | Kryndel source parses JSON values and constructs verifier-compatible bytecode records including numeric jump targets under the Python VM; full schema parity, file input, and production KEXE decoding remain Python |
