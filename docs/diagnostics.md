...
The categories are `lex`, `parse`, `type-mismatch`, `runtime`, `artifact`, `cli`, `io`, and `resource`. A diagnostic contains the filename, one-based line and column, a concise message, and a source excerpt with a caret whenever source text is available. The first error is retained so output remains deterministic.
...
| `69` | The launcher could not find a C11 compiler. |
