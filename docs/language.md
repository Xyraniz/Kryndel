...
Direct self-recursive calls in tail position are compiled to a runtime trampoline. A return such as `return count(n - 1, acc + 1)` reuses the current logical call frame, so it does not consume another call-depth slot. Calls must be the complete returned expression; recursion hidden inside arithmetic, collection construction, or deferred work remains an ordinary checked call.
