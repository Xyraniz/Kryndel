...
Install it alongside the regular client with `kry add discord-self ^1.5.0`; it depends on `discord ^2.3.0`. Import both `discord` and `discord-self` when an application needs both APIs. The modules have separate package paths. Along with its existing message and Gateway APIs, `Client` exposes forty-six `discord.py-self`-style REST methods. The first ten are `fetch_guild_preview`, `fetch_guild_profile`, `fetch_stage_instance`, `fetch_user_profile`, `fetch_private_channels`, `fetch_relationships`, `fetch_connections`, `fetch_notes`, `fetch_note`, and `edit_note`. They return Kryndel `Json` values instead of Python model objects; `edit_note` sends an empty note to clear it. `fetch_guilds(with_counts)` includes approximate counts by default; pass `false` to omit them. `guilds()` remains an alias that omits counts.

Version 1.4.0 adds `fetch_invite`, `fetch_widget`, `fetch_sticker`, `fetch_sticker_pack`, `fetch_member_verification`, `fetch_join_request`, `join_request_guilds`, `fetch_settings`, `fetch_tracking_settings`, and `friend_suggestions`. The expanded 1.5.0 release adds `fetch_template`, `fetch_country_code`, `fetch_location_info`, `applications`, `detectable_applications`, `fetch_application`, `fetch_partial_application`, `fetch_public_application`, `fetch_public_applications`, `teams`, `fetch_team`, `fetch_company`, `fetch_entitlements`, `fetch_sku`, `fetch_store_listing`, `fetch_eula`, `fetch_gift`, `fetch_published_store_listing`, `fetch_published_store_listings`, `fetch_primary_store_listing`, `fetch_primary_store_listings`, `fetch_sku_subscription_plans`, `fetch_skus_subscription_plans`, `fetch_live_build_ids`, `price_tiers`, and `giftable_entitlements`. Batch methods accept arrays of IDs, validate each one, cap batches at 100, and return empty JSON collections without a request for empty inputs. `fetch_live_build_ids` sends only the requested numeric branch IDs. Gift and template methods accept codes rather than full URLs; optional country codes and snowflake path/query values are validated. These methods return Kryndel `Json`; `fetch_settings` unwraps the `settings` object.
