...
Install it alongside the regular client with `kry add discord-self ^1.4.0`; it depends on `discord ^2.3.0`. Import both `discord` and `discord-self` when an application needs both APIs. The modules have separate package paths. Along with its existing message and Gateway APIs, `Client` exposes twenty `discord.py-self`-style REST methods. The first ten are `fetch_guild_preview`, `fetch_guild_profile`, `fetch_stage_instance`, `fetch_user_profile`, `fetch_private_channels`, `fetch_relationships`, `fetch_connections`, `fetch_notes`, `fetch_note`, and `edit_note`. They return Kryndel `Json` values instead of Python model objects; `edit_note` sends an empty note to clear it. `fetch_guilds(with_counts)` includes approximate counts by default; pass `false` to omit them. `guilds()` remains an alias that omits counts.

Version 1.4.0 adds `fetch_invite`, `fetch_widget`, `fetch_sticker`, `fetch_sticker_pack`, `fetch_member_verification`, `fetch_join_request`, `join_request_guilds`, `fetch_settings`, `fetch_tracking_settings`, and `friend_suggestions`. These return JSON values; `fetch_settings` unwraps the `settings` object. Invite methods accept invite codes rather than full URLs and validate code characters; snowflake arguments are validated before requests.
