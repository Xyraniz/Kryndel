...
Install it alongside the regular client with `kry add discord-self ^1.3.0`; it depends on `discord ^2.3.0`. Import both `discord` and `discord-self` when an application needs both APIs. The modules have separate package paths. Along with its existing message and Gateway APIs, `Client` now exposes ten `discord.py-self`-style endpoints: `fetch_guild_preview`, `fetch_guild_profile`, `fetch_stage_instance`, `fetch_user_profile`, `fetch_private_channels`, `fetch_relationships`, `fetch_connections`, `fetch_notes`, `fetch_note`, and `edit_note`. These endpoints return Kryndel `Json` values instead of Python model objects; `edit_note` sends an empty note to clear it. `fetch_guilds(with_counts)` includes approximate counts by default; pass `false` to omit them. `guilds()` remains an alias that omits counts.
