...
The native backend accepts `windows-x64` and `windows-arm64` as PE header targets; the PE32+ artifact contains a real minimal entry point and an `ExitProcess` import. `kry inspect` validates the `MZ`/`PE\0\0` signatures, architecture, and section count without executing the file. Generating a complete Kryndel executable with every feature still requires broader IR lowering and native runtime linking; until then, the `KRYNATIVE4` bundle remains the complete portable path.
