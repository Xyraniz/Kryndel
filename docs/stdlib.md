...
| `discord_api_request` | `discord_api_request(method: String,path: String,body: String,token: String) -> Result[String,String]` | Calls only the fixed Discord API v10 HTTPS origin using `Authorization: Bot`, validates JSON bodies, bounds request and response sizes, redacts tokens, shares route/global rate-limit buckets, and retries HTTP 429 up to five times (maximum five minutes per retry). |
| `discord_interaction_request` | `discord_interaction_request(method: String,path: String,body: String) -> Result[String,String]` | Calls only `/interactions` or `/webhooks` on Discord's fixed API origin without a bot Authorization header; validates JSON and redacts long route tokens. |
| `discord_api_upload` | `discord_api_upload(method: String,path: String,payload: String,filename: String,data: Bytes,token: String) -> Result[String,String]` | Sends one multipart file and `payload_json` to a fixed Discord API route, bounds body and response size, and replays the body for bounded rate-limit retries. |
| `discord_api_upload_files` | `discord_api_upload_files(method: String,path: String,payload: String,filenames: Array[String],files: Array[Bytes],token: String) -> Result[String,String]` | Sends one to ten multipart files and `payload_json` to a fixed Discord API route with Bot authentication; filenames and total body size are bounded. |
| `discord_verify_interaction` | `discord_verify_interaction(public_key: String,timestamp: String,body: String,signature: String) -> Result[Bool,String]` | Checks Discord Ed25519 interaction signatures and rejects timestamps outside the five-minute replay window. |
| `discord_cache_get`, `discord_cache_put`, `discord_cache_delete`, `discord_cache_clear`, `discord_cache_ingest` | Bounded Discord object cache. | Keeps up to 10,000 recent JSON objects for 30 minutes; Gateway dispatches update cached users, guilds, channels, members, roles, messages, threads, and voice states. |
| `websocket_send_binary` | `websocket_send_binary(socket: WebSocket,data: Bytes) -> Result[Nil,String]` | Sends one bounded RFC 6455 binary message. |
| `websocket_receive_binary`, `websocket_receive_binary_timeout` | Binary WebSocket message reads. | Assemble fragmented binary messages and retain partial frame bytes after a timeout. |
| `websocket_connect`, `websocket_send`, `websocket_receive`, `websocket_close` | RFC 6455 text WebSocket lifecycle. | Require `ws`/`wss`, validate the handshake, mask client frames, assemble fragmented messages, bound payloads, and expose `WebSocket` as non-Copy. |
| `websocket_receive_timeout` | `websocket_receive_timeout(socket: WebSocket,milliseconds: Int) -> Result[String,String]` | Wait for one complete text message; a timeout does not discard partial frame bytes. Durations must be in `1..300000` milliseconds. |
| `process_run` | `process_run(program: String, args: Array[String]) -> Result[Int,String]` | Starts a program directly without a shell and bounds combined output. Unavailable with `--restricted`. |
| `process_args` | `process_args() -> Array[String]` | Under `kry run`, returns arguments passed after the source path and does not expose the host command line. A self-hosted Windows PE uses the native process arguments after `argv[0]`, decoded into UTF-8 strings. Unavailable with `--restricted`. |
...
| `sqlite_open`, `sqlite_exec`, `sqlite_query`, `sqlite_close` | Embedded SQLite database handles. | The pure-Go SQLite engine runs inside the interpreter; paths are sandboxed, `:memory:` is supported, query rows are bounded, and cells are returned as text. Unavailable with `--restricted` because SQL can attach another database path. |
...
| `ffi_library_open`, `ffi_symbol`, `ffi_library_close`, `ffi_thread_pin`, `ffi_thread_unpin` | Dynamic-library, symbol, and OS-thread controls. | Uses `purego` without CGO on Linux, macOS, FreeBSD, and Windows; thread pinning keeps the current runtime goroutine on its OS thread until the matching unpin. |
| `ffi_buffer_new`, `ffi_buffer_new_sized`, `ffi_buffer_address`, `ffi_buffer_read`, `ffi_buffer_close` | NUL-terminated native-call buffers. | `ffi_buffer_new_sized` allocates a zeroed output buffer within the runtime string-byte limit. Returned addresses are opaque tokens valid only as `p` arguments to `ffi_call`; Kryndel never exposes raw pointer values to user code. |
...
| `process_list` | `process_list() -> Result[Json,String]` | Enumerates processes through gopsutil, sorts by PID, and returns JSON metadata (`pid`, `name`, `exe`, `username`, `create_time_ms`, `status`) up to `MaxArrayElements`. It returns an error instead of silently truncating the list when the cap is exceeded. Permission-limited fields are empty rather than fabricated. |
| `geocode_ip` | `geocode_ip(ip: String) -> Result[Json,String]` | Validates an IPv4/IPv6 address, queries `KRY_GEOCODER_ENDPOINT` or the default `https://ipapi.co`, applies the wall-clock and response-size limits, and returns canonical JSON. |
| `screen_display_count`, `screen_display_bounds` | Display enumeration helpers. | Use the desktop API through `kbinani/screenshot`; bounds include virtual-desktop coordinates and dimensions. |
| `screen_capture`, `screen_capture_display` | Screen capture helpers. | Capture a region or an active display as PNG `Bytes`, subject to the output limit; API/display errors are returned rather than fabricated images. |
| `camera_capture` | `camera_capture(device: String,width: Int,height: Int) -> Result[Bytes,String]` | Captures one webcam frame as PNG. Windows uses the installed `ffmpeg` DirectShow backend (`KRYNDEL_FFMPEG` can select its path); Linux uses V4L2 directly. Device/capability errors are returned. |
| `win_input_read` | `win_input_read(kind: String,timeout_ms: Int) -> Result[Json,String]` | Installs a bounded Windows low-level keyboard or mouse hook, pumps the Win32 message queue, returns the first event as JSON, and unhooks it on timeout or event. Non-Windows targets return an explicit platform error. |
