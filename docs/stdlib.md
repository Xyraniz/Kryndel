...
| `core/json` | `parse(String) -> JsonResult`, `decode_bytecode(String) -> BytecodeResult`, `decode_bytecode_file(String) -> BytecodeResult` | Source-level JSON value parser plus bounded v1 bytecode schema decoder using controlled `fs.read_text`; full schema validation, file input ownership, and production KEXE decoding remain Python |
