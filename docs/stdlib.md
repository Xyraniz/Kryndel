...
| `core/backend` | `seed_module()`, `emit(ModuleRecord, String) -> BackendResult`, `emit_exit(ModuleRecord, String, Int) -> BackendResult` | Deterministic x86_64 Linux exit-status assembly seed for an empty main module with status 0..255; complete native backend, object format, linking, and compiler/runtime integration remain pending |
