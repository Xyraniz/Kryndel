...
| `process_args` | `process_args() -> Array[String]` | Under `kry run`, returns arguments passed after the source path and does not expose the host command line. A self-hosted Windows PE uses the native process arguments after `argv[0]`, decoded into UTF-8 strings. |
