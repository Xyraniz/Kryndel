...
| `core/json` | `parse(String) -> JsonResult`, `decode_bytecode(String) -> BytecodeResult` | Source-level JSON value parser plus bounded v1 bytecode schema decoder for scalar constants and PUSH_CONST/PUSH_NIL/LOAD/STORE/POP/RETURN; full schema validation, file input, and production KEXE decoding remain Python |
| `core/backend` | `seed_module()`, `emit(ModuleRecord, String) -> BackendResult`, `emit_exit(ModuleRecord, String, Int) -> BackendResult`, `emit_program(ModuleRecord, String) -> BackendResult` | Deterministic x86_64 Linux exit-status assembly for an empty main and one scalar `PUSH_CONST`/`RETURN` program with status 0..255; general native backend, object format, linking, and compiler/runtime integration remain pending |
